...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, ButtonModule, InputTextModule, InputNumberModule, DialogModule, TableModule, TagModule, ToastModule, CheckboxModule, PasswordModule, ConfirmDialogModule],
    ...
    <div class="p-6">
      <div class="flex items-center justify-between mb-6">
        <div>
          ...
          <p class="text-sm text-gray-500 mt-1">Configure your email accounts for IMAP/SMTP access</p>
        </div>
        <div class="flex gap-2">
          ...
          <p-button label="Add Account" icon="pi pi-plus" (onClick)="openDialog()" />
        </div>
      </div>

      <p-table [value]="accounts()" [loading]="loading()" styleClass="p-datatable-sm">
        <ng-template pTemplate="header">
          <tr><th>Label</th><th>Email</th><th>IMAP</th><th>SMTP</th><th>Status</th><th>Last Sync</th><th style="width:180px">Actions</th></tr>
        </ng-template>
        <ng-template pTemplate="body" let-a>
          <tr>
            <td class="font-medium">{{ a.label }}</td>
            <td>{{ a.emailAddress }}</td>
            <td class="text-sm">{{ a.imapHost }}:{{ a.imapPort }}</td>
            <td class="text-sm">{{ a.smtpHost }}:{{ a.smtpPort }}</td>
        ...
        <ng-template pTemplate="emptymessage">
          <tr><td colspan="7" class="text-center text-gray-400 py-8">No mail accounts configured. Add one to get started.</td></tr>
        </ng-template>
      </p-table>
    </div>

    <p-dialog [header]="editId ? 'Edit Mail Account' : 'Add Mail Account'" [(visible)]="dialogVisible" [modal]="true" [style]="{width:'520px', 'max-width': '95vw'}" [contentStyle]="{'overflow-x': 'hidden'}">
...
export class MailSettingsComponent implements OnInit {
  ...
  testConnection(account: MailAccount): void {
    ...
    this.http.post<{ data: { imap: boolean; error?: string } }>(`${environment.apiBaseUrl}/mail/accounts/${account.id}/test`, {}).subscribe({
      next: (r) => {
        this.testingId = null;
        if (r.data.imap) this.msg.add({ severity: 'success', summary: 'Connection OK', detail: 'IMAP connected successfully' });
