...
/**
 * Embeddable activity panel — shows activities linked to a specific entity.
 *
 * Usage:
...
@Component({
  ...
  template: `
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
      <div class="flex items-center justify-between mb-4">
        <h3 class="text-lg font-semibold text-gray-700 flex items-center gap-2">
          <i class="pi pi-calendar text-indigo-600"></i> Activities
        </h3>
        <p-button label="Add Activity" icon="pi pi-plus" size="small" (onClick)="openDialog()" />
      </div>
      ...
      <p-table [value]="activities()" styleClass="p-datatable-sm" [rows]="10" [paginator]="activities().length > 10">
        <ng-template pTemplate="header">
          <tr>
            <th style="width:32px"></th>
            <th>Subject</th><th>Type</th><th>Category</th><th>Date / Time</th><th>Status</th><th>Owner</th><th style="width:90px">Actions</th>
          </tr>
        </ng-template>
    ...
    <!-- Add / Edit Dialog -->
    <p-dialog [header]="editId ? 'Edit Activity' : 'New Activity'" [(visible)]="dialogVisible" [modal]="true" [style]="{width:'550px'}">
...
export class ActivityPanelComponent implements OnInit {
  ...
  loadActivities(): void {
    const params = [`entityType=${this.entityType}`, `entityId=${this.entityId}`, 'limit=200'];
  ...
  saveActivity(): void {
    ...
    const body: Record<string, unknown> = {
      ...
      // Auto-link to the parent entity
      associations: [{ entityType: this.entityType, entityId: this.entityId }],
    };
