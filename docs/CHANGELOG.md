...
### Added (2026-05-11/13 — Session 15: OEM Catalog, Price Lists, Bid Eval Fixes, Excel/AI Import)
- **OEM Catalog & Price Lists** — Two-tier item model: existing `Item` serves as Master Item (generic product), new `OemCatalogItem` links vendor-specific products (model, part no, brand, price, specs, warranty, MOQ) to master items. New `VendorPriceList` model groups imports by vendor + date. Prisma schema: 2 new models + optional `oemCatalogItemId` FK on `VendorQuotationLine` and `PoLine`. API: full CRUD `/api/v1/oem-catalog` (+ `/by-master-item/:id` for rate comparison, `/meta/brands`), full CRUD `/api/v1/price-lists` (+ import preview/extract/execute). Upsert on import: matches by `(vendorId, oemPartNo)` — creates new or updates price.
- **Price List Import Wizard** — 4-step flow: (1) Select vendor + name + date + upload file, (2) Map system fields → Excel columns with inline default values for unmapped fields, (3) Link rows to Master Items — auto-links from existing OEM catalog by part no/model name, auto-link by name match, create-all-unlinked bulk action, per-row create button, (4) Preview + import. Fetches all rows (not just sample 5). Supports Excel (.xlsx/.xls/.csv) and image/PDF via Gemini AI extraction.
- **Quotation Excel/AI Import** — Enhanced with same inverted column mapping UI (system fields as rows), default value inputs, image/PDF support via Gemini extraction.
- **AI Price List Extraction** — New `extractPriceListFromFile()` in Gemini lib. Sends image/PDF to Gemini with structured prompt to extract tabular price list data. Returns headers + rows in same format as Excel parse. Endpoints: `POST /price-lists/import/extract`, `POST /quotations/import/extract`.
- **OEM Catalog Picker in PO + Quotation** — PO line item dialog: new autocomplete "Pick from OEM Catalog" that searches by model/part no/brand, auto-fills description, UOM, rate, GST, HSN. Quotation lines: search icon next to description opens OEM catalog picker dialog.
- **Item Category from Lookup Lists** — Item categories now configurable via Admin → Lookup Lists (`item_category`) instead of hardcoded. Seeded with 10 default categories.
- **Item list OEM count** — New "OEM Options" column showing count of OEM catalog items per master item, clickable to filtered OEM catalog page.
- **Sidebar nav** — Added "Price Lists" and "OEM Catalog" under Procurement.

### Fixed (2026-05-11/13)
- **Bid Eval: dropdown truncation** — Technical scoring table uses `table-fixed` layout with 50% max width for Scoring column. Selected item text properly truncates with ellipsis.
- **Bid Eval: itemized mode persistence** — Zod schema on API was stripping `expanded`, `subScores`, `selectedSubId`, `manual`, `label`, `maxMarks` fields. Fixed schema to include all fields. Also persists `manualSections` and PQ manual fields.
- **Bid Eval: sub-score zero on blur** — Root cause: `sub.maxMarks` was 0 for AI-extracted criteria, causing `Math.min(value, 0)` to force everything to zero. Now falls back to section `maxMarks`. Added section total cap so sub-scores can't exceed section max.
- **Bid Eval: single sub-criterion** — Sections with exactly 1 sub-criterion now show text directly instead of a dropdown, with an editable score input.
- **Gemini output tokens** — Bumped from 16K to 65K for large RFPs (120+ pages). Default model upgraded to `gemini-2.5-flash`.

### Added (2026-05-09 — Session 14: Procurement Overhaul Phase 1 — Approval Engine, Vendor KYC, Material Request)
- **Generic Approval Engine** — Reusable multi-step approval workflow system. 4 new models (`ApprovalWorkflow`, `ApprovalWorkflowStep`, `ApprovalRequest`, `ApprovalAction`). Service layer (`submitForApproval()`, `handleApprovalAction()`) with self-approval prevention, consecutive-step same-actor prevention, and value-based step filtering. Notifications to approvers via role or specific user. API: `GET/POST/PATCH/DELETE /api/v1/approvals/workflows`, `GET /api/v1/approvals/requests` (with `myPending` filter), `POST /api/v1/approvals/requests/:id/act` (APPROVE/REJECT/RETURN). Angular: Approval Inbox page (sidebar top-level) with pending/all tabs, approve/reject/return dialog. Approval Workflows admin page under System.
- **Vendor KYC Lifecycle** — Vendor model enhanced with `vendorCode` (auto-generated VND-FY-XXXX), `kycStatus` (DRAFT → KYC_PENDING → ACTIVE → BLOCKED → BLACKLISTED), `msmeNumber`, `msmeCategory`, `tdsSection`, `defaultTdsRateBps`, `isOem`, `oemPrincipals`. New `VendorDocument` model (typed: GST_CERT, PAN_CARD, MSME, CANCELLED_CHEQUE, NDA, ISO, OEM_AUTH, COMPANY_REG, OTHER) with file upload via DMS. New `VendorBankDetail` model with dual-control verification (verifier ≠ creator). GSTIN checksum regex and PAN format validation (`[A-Z]{5}[0-9]{4}[A-Z]`). Duplicate GSTIN/PAN detection on create. API: `POST/GET/DELETE /vendors/:id/documents`, `POST/PATCH/DELETE /vendors/:id/bank-details`, `POST /vendors/:id/bank-details/:bdId/verify`, `POST /vendors/:id/submit-kyc|activate|block|unblock`. Vendor detail page enhanced with KYC status badges, document upload panel, bank detail management with verify button.
- **Material Request (Purchase Requisition)** — Full implementation of MR module (schema existed, no routes). Schema enhanced: added `title`, `source` (STOCK/PURCHASE/FABRICATION/MAINTENANCE/EMERGENCY), optional `projectId`. Line items enhanced: `description`, `uom`, `estimatedRatePaise`, optional `itemId` (supports free-text items), `sortOrder`. Full API: `CRUD /api/v1/material-requests` with nested lines, `POST /:id/submit` (integrates with approval engine), `POST /:id/approve|reject|return`, line CRUD restricted to DRAFT status. Auto-number: MR-FY-XXXX. Angular: MR list page (summary cards, status/BU/search filters, table) and MR detail page (create/edit/view with line item dialog, approval action buttons based on status).
- **Procurement sidebar** — "Material Requests" added as first item in Procurement nav group. "Approvals" added as top-level sidebar item. "Approval Workflows" added under System.

### Fixed (2026-05-09)
- **IMAP crash fix** — Added error handler to `ImapFlow` client in `imap.service.ts` to prevent unhandled `ECONNRESET` from crashing the API process during mail sync.

### Added (2026-04-29 — Session 10: GDrive DMS, Bid Evaluation, Mail Attachments to DMS)
- **Google Drive DMS Migration** — All document storage (company docs, tender docs, project docs, expense attachments, travel proofs) migrated to Google Drive via OAuth. Folder structure: `Shared/{Company, Tenders/{id}, Projects/{id}, Expenses/{id}, Travel/{id}/{Tickets,Hotels,Expenses}}`, `Private/{User}`. Migration script (`scripts/migrate-to-gdrive.js`) creates folders + uploads files + updates DB. All upload/download routes refactored to use `dms-storage.ts` layer with `downloadFileWithFallback` for backward compatibility with legacy local files.
- **Bid Evaluation (PQ + Technical Scoring)** — New collapsible "Bid Evaluation" card on Tender detail page. AI-extracted or manually entered Pre-Qualification criteria table (criterion, category, mandatory flag, meets/doesn't-meet toggle, remarks). Technical Evaluation scoring table (sections → sub-criteria, max marks, expected score input, scoring method). Threshold banner comparing expected score vs minimum qualifying score. Progress bar visualization. Go/No-Go decision with remarks. Saved in `bidEvaluation` JSON field on Tender (survives re-analysis). New API: `GET/PUT /tenders/:id/bid-evaluation`.
- **Enhanced AI RFP Analysis** — Gemini prompt now extracts structured `preQualification` array (id, criterion, category, requirement, clause, mandatory) and `technicalEvaluation` object (sections, sub-criteria, maxMarks, scoringMethod, minimumQualifyingScore). `maxOutputTokens` bumped to 16384. Fixed `analyzeRfp()` to read files from GDrive via `downloadFileWithFallback` instead of local `fs.readFileSync`.
- **Mail: Save Attachment to DMS** — "Save to Documents" (folder-plus icon) button on each email attachment in mail reader. Opens dialog with: file preview, editable name, DMS folder tree picker, optional entity link (Opportunity/Project/Tender/Initiative/Account). API `POST /mail/messages/:id/attachment/:idx/save-to-dms` fetches from IMAP → writes to temp → uploads via storage layer → creates Document record + optional entity link.
- **Document Browser enhancements** — Folder tree width increased 50% (288→432px). Files viewable in browser tab (not just download). Loading spinner on file icon during fetch. Delete button for empty folders. Close button on Linked Entities panel.
- **Configuration: OAuth labels** — Updated Document Storage section labels from "Service Account" to "OAuth Credentials" with correct instructions.

### Fixed (2026-04-29)
- **Activity "Linked To" visibility** — Replaced invisible `bg-gray-100` span with PrimeNG `p-tag severity="secondary"` for dark theme compatibility.
- **Gemini 503 handling** — Added error message for 503 (model overloaded) with suggestion to switch models.
